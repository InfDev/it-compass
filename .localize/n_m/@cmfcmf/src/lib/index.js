...
function urlMatchesPrefix(url, prefix) {
    if (prefix.startsWith("/")) {
        throw new Error(`prefix must not start with a /. This is a bug (url: "${url}", prefix: ${prefix}).`);
    }
    if (prefix.endsWith("/")) {
        throw new Error(`prefix must not end with a /. This is a bug (url: "${url}", prefix: ${prefix}).`);
    }
...
const languageSchema = utils_validation_1.Joi.string().valid("ar", "da", "de", "en", "es", "fi", "fr", "hi", "hu", "it", "ja", "nl", "no", "pt", "ro", "uk", "ru", "sv", "th", "tr", "vi", "zh");
            ...
            const data = routesPaths
                ...
                if (indexDocs) {
                    for (const docsPlugin of docsPlugins.values()) {
                        const docsBasePath = trimLeadingSlash(trimTrailingSlash(docsPlugin.options.routeBasePath));
                        const docsTagsPath = trimLeadingSlash(trimTrailingSlash(docsPlugin.options.tagsBasePath));
                ...
                if (indexBlog) {
                    for (const blogPlugin of blogPlugins.values()) {
                        const blogBasePath = trimLeadingSlash(trimTrailingSlash(blogPlugin.options.routeBasePath));
                        const blogTagsPath = trimLeadingSlash(trimTrailingSlash(blogPlugin.options.tagsBasePath));
                ...
                if (indexPages) {
                    for (const pagesPlugin of pagesPlugins.values()) {
                        const pagesBasePath = trimLeadingSlash(trimTrailingSlash(pagesPlugin.options.routeBasePath));
                    ...
                    rules: [
                        {
                            ...
                            use: [
                                ...
                                {
                                    loader: path_1.default.join(__dirname, "lunr-generator.js"),
                                    options: { generated },
                                },
                            ],
                        },
